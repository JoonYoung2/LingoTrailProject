...
const insertCheck = () => {
    ...
    let msg = "";
    fetch("/blank_question/language_insert_list",
        {
            method: "get"
        })
        .then(res => res.json())
        .then(result => {
            ...
            msg = `
            <form action="/blank_question/language_insert" method="post" id="formId">
...
const updateCheck = () => {
    ...
    let updateInputListLanguage = document.querySelectorAll(".speakLanguage");

    
    for (var i = 0; i < updateInputListId.length; i++) {
        if (updateInputListId[i].checked == true) {
            if(updateInputListLanguage[i].value == ""){
                alert("입력되지 않은 언어가 있습니다.");
                updateInputListLanguage[i].focus();
                return;
            }
    ...
    fetch("/blank_question/language_update",
        {
            method: "post",
...
const deleteCheck = () => {
    ...
    if (window.confirm("Speak Game에 연관된 모든 List가 함께 삭제됩니다. \n\n삭제하시겠습니까?")) {
    ...
    fetch("/blank_question/language_delete",
        {
            method: "post",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(data)
